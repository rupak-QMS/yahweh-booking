...
const DEF_BOOKINGS = [
  { id: "YPC1001", clientId: "c1", clientName: "Sarah Mitchell", clientEmail: "sarah@example.com", service: "House Cleaning", date: "2026-05-25", time: "9:00 AM", address: "12 Rose St, Blacktown NSW 2148", total: 117.81, status: "Confirmed", freq: "Weekly", extras: [] },
  ...
  { id: "YPC1003", clientId: "c2", clientName: "James Lee", clientEmail: "james@example.com", service: "Office Cleaning", date: "2026-05-28", time: "8:00 AM", address: "45 Market St, Sydney NSW 2000", total: 152.90, status: "Pending", freq: "Fortnightly", extras: [] },
...
function Logo({ small }) {
  return (
    <img
      ...
      style={{ height: small ? 52 : 72, width: "auto", objectFit: "contain", display: "block", mixBlendMode: "multiply" }}
    />
  );
}
...
function BookingApp({ user, services, extras, coupons, freqs: freqsProp, onComplete }) {
  ...
  function submit() {
    const nb = {
      ...
      clientName: [bk.firstName, bk.lastName].join(" "),
      clientEmail: bk.email,
      service: bk.svc.name,
      date: bk.date ? bk.date.toISOString().split("T")[0] : "",
      time: bk.time,
      address: [bk.address, bk.suburb, bk.state, bk.postcode].filter(Boolean).join(", "),
      total, status: "Confirmed", freq: bk.freq.label,
  ...
  if (done) return (
    <div style={{ maxWidth: 540, margin: mobile ? "0 auto" : "48px auto", padding: "16px" }}>
  ...
  return (
    <div style={{ paddingBottom: mobile ? 130 : 0 }}>
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: mobile ? "16px" : "24px 28px", display: mobile ? "block" : "grid", gridTemplateColumns: "1fr 300px", gap: 24 }}>
        <div>
          {/* STEP 1 */}
          {step === 1 && <>
            ...
            <div style={S.panel(mobile)}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {extras.map(ex => {
                  const a = bk.extras.some(e => e.id === ex.id);
                  return (
          ...
          {step === 2 && (
            <div style={S.panel(mobile)}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: mobile ? 4 : 6, marginBottom: 20 }}>
                ...
                {dates.map((d, i) => {
                  const a = bk.date && d.toDateString() === bk.date.toDateString();
              ...
              <div style={{ display: "grid", gridTemplateColumns: mobile ? "repeat(3,1fr)" : "repeat(auto-fill,minmax(100px,1fr))", gap: mobile ? 8 : 10 }}>
                {TIME_SLOTS.map(t => {
                  const a = bk.time === t;
                  return <div key={t} onClick={() => setV("time", t)} style={{ border: `2px solid ${a ? BLUE : BORDER}`, borderRadius: 10, padding: mobile ? "11px 6px" : "12px", textAlign: "center", cursor: "pointer", background: a ? BLUE : WHITE, color: a ? WHITE : MUTED, fontWeight: a ? 800 : 500, fontSize: mobile ? 12 : 13 }}>{t}</div>;
                })}
              </div>
          ...
          {step === 4 && (
            <div style={S.panel(mobile)}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: MUTED }}>🔒 Protected by 256-bit SSL encryption</div>
            </div>
          )}
          ...
          {!mobile && (
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
              {step > 1 ? <button style={{ ...S.btn(WHITE, BLUE, BLUE), display: "flex", alignItems: "center", gap: 6 }} onClick={() => { setStep(s => s - 1); window.scrollTo(0, 0); }}>← Back</button> : <span />}
              {step < 5 ? <button style={{ ...S.btn(BLUE, WHITE), display: "flex", alignItems: "center", gap: 6 }} onClick={next}>Continue →</button> : <button style={{ ...S.btn(GREEN, WHITE), display: "flex", alignItems: "center", gap: 8, fontSize: 15 }} onClick={next}>✅ Confirm & Pay {fmt(total)}</button>}
            </div>
          )}
        </div>
      ...
      <div style={{ background: `linear-gradient(135deg, ${BLUE} 0%, #1565a8 60%, #0d4a7a 100%)`, padding: mobile ? "24px 16px 80px" : "32px 40px 90px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 900, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={onBook} style={{ background: GREEN, color: WHITE, border: "none", borderRadius: 10, padding: mobile ? "10px 16px" : "12px 22px", fontSize: mobile ? 13 : 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                + New Booking
              </button>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: mobile ? "16px 16px 0" : "24px 0 0" }}>
        ...
        <div style={{ display: "flex", gap: 4, marginBottom: 16, background: WHITE, borderRadius: 12, padding: 6, border: `1px solid ${BORDER}` }}>
          {[["upcoming", `Upcoming (${upcoming.length})`], ["past", `History (${past.length})`]].map(([id, label]) => (
            <button key={id} onClick={() => setActiveTab(id)} style={{ flex: 1, background: activeTab === id ? BLUE : "transparent", color: activeTab === id ? WHITE : MUTED, border: "none", borderRadius: 9, padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{label}</button>
          ))}
        </div>
        ...
        <div style={{ marginTop: 24 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr 1fr" : "repeat(3,1fr)", gap: 12 }}>
            {[
              ...
              <div key={label} onClick={action} style={{ background: WHITE, borderRadius: 14, border: `1px solid ${BORDER}`, padding: "16px", cursor: "pointer", display: "flex", alignItems: "center", gap: 12 }}>
...
function AdminDash({ admins, setAdmins, bookings, setBookings, clients, setClients, services, setServices, extras, setExtras, coupons, setCoupons, freqs, setFreqs, onLogout }) {
  ...
  function inviteAdmin() {
    ...
    const current = (() => { try { return JSON.parse(sessionStorage.getItem("adminList") || "[]"); } catch (e) { return []; } })();
    if (current.find(a => a.email.toLowerCase() === inviteForm.email.toLowerCase())) { alert("Email already exists"); return; }
  ...
  function changeOwnPw() {
    ...
    if (changePwForm.newPw.length < 6) { setChangePwErr("New password must be at least 6 characters."); return; }
  ...
  const NavContent = () => (
    <>
      <div style={{ padding: "0 20px 16px", borderBottom: `1px solid ${BORDER}`, marginBottom: 12 }}>
        {isSuperAdmin
          ? <div style={{ background: `linear-gradient(135deg,${BLUE},#2196f3)`, borderRadius: 8, padding: "6px 12px", display: "inline-flex", alignItems: "center", gap: 5 }}><span style={{ fontSize: 11, color: WHITE, fontWeight: 800 }}>⭐ Super Admin — {adminUser?.name}</span></div>
  ...
  return (
    <div style={{ display: "flex", minHeight: "calc(100vh - 82px)" }}>
      ...
      {inviteModal && <Modal title="Invite New Admin" onClose={() => setInviteModal(false)}>
        ...
        <div style={{ background: LIGHT_BLUE, borderRadius: 9, padding: "10px 14px", fontSize: 13, color: "#1a3d60", marginBottom: 14 }}>ℹ️ Admin can change their password after first login.</div>
        <button style={{ ...S.btn(GREEN, WHITE), width: "100%", padding: "14px" }} onClick={inviteAdmin}>Create Admin Account</button>
      </Modal>}
      ...
      {changePwModal && <Modal title="Change My Password" onClose={() => { setChangePwModal(false); setChangePwErr(""); setChangePwOk(false); }}>
        ...
        {changePwOk && <div style={{ color: GREEN, fontSize: 13, marginBottom: 10, background: LIGHT_GREEN, borderRadius: 8, padding: "8px 12px" }}>✅ Password changed successfully!</div>}
        <button style={{ ...S.btn(BLUE, WHITE), width: "100%", padding: "14px" }} onClick={changeOwnPw}>Update Password</button>
      </Modal>}
      ...
      {resetPwModal && <Modal title={`Reset Password — ${resetPwModal.name}`} onClose={() => { setResetPwModal(null); setResetPwForm({ newPw: "", confirm: "" }); }}>
        ...
        <button style={{ ...S.btn(BLUE, WHITE), width: "100%", padding: "14px" }} onClick={resetAdminPw}>Reset Password</button>
      </Modal>}
      ...
      {/* Desktop sidebar */}
      {!mobile && (
        <div style={{ width: 230, background: WHITE, borderRight: `1px solid ${BORDER}`, padding: "24px 0", flexShrink: 0 }}>
          <NavContent />
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: "auto", background: BG, paddingBottom: mobile ? 70 : 0 }}>
        <div style={{ background: WHITE, borderBottom: `1px solid ${BORDER}`, padding: mobile ? "14px 16px" : "20px 28px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 100 }}>
          ...
          {!isSuperAdmin && !mobile && (
            <button onClick={() => setChangePwModal(true)} style={{ ...S.btn(LIGHT_BLUE, BLUE), fontSize: 13, padding: "8px 16px" }}>🔒 Change Password</button>
          )}
        </div>
        ...
        <div style={{ padding: mobile ? "12px 16px" : "24px 28px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: mobile ? 10 : 14, marginBottom: mobile ? 16 : 28 }}>
            {[
              { icon: "📋", label: "Bookings", value: bookings.length, color: BLUE, bg: LIGHT_BLUE },
              { icon: "💰", label: "Revenue", value: fmt(revenue), color: GREEN, bg: LIGHT_GREEN },
              { icon: "⏳", label: "Pending", value: bookings.filter(b => b.status === "Pending").length, color: "#e67e22", bg: "#fff8f0" },
              { icon: "👥", label: "Clients", value: clients.length, color: BLUE, bg: LIGHT_BLUE },
          ...
          {tab === "admins" && isSuperAdmin && <>
            <div style={{ background: `linear-gradient(135deg,${BLUE},#2196f3)`, borderRadius: 14, padding: "20px 24px", marginBottom: 20, color: WHITE }}>
              ...
              <div style={{ fontSize: 13, opacity: 0.8, marginTop: 4 }}>This section is hidden from all other admins.</div>
            </div>
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
              <button style={{ ...S.btn(GREEN, WHITE), display: "flex", alignItems: "center", gap: 6 }} onClick={() => setInviteModal(true)}>
                + Invite New Admin
              </button>
            </div>
            {admins.length === 0 ? (
                ...
                {admins.map(a => (
                  <div key={a.id} style={{ background: WHITE, borderRadius: 12, border: `1px solid ${BORDER}`, padding: "16px 20px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <span style={{ background: LIGHT_BLUE, color: BLUE, borderRadius: 50, padding: "2px 10px", fontSize: 11, fontWeight: 700, marginTop: 4, display: "inline-block" }}>Normal Admin</span>
                    </div>
                ...
                return (
                  <div key={c.id} style={{ background: WHITE, borderRadius: 12, border: `1px solid ${BORDER}`, padding: "14px" }}>
                    ...
                    <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
                      ...
                      <div style={{ flex: 1, background: BG, borderRadius: 10, padding: "10px", textAlign: "center" }}>
                        <div style={{ fontSize: 20, fontWeight: 900, color: GREEN }}>{fmt(cbks.reduce((s, b) => s + b.total, 0))}</div>
                ...
                <div key={svc.id} style={{ background: WHITE, borderRadius: 12, border: `1px solid ${BORDER}`, overflow: "hidden" }}>
                  <div style={{ background: `linear-gradient(135deg,${LIGHT_BLUE},${WHITE})`, padding: "18px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div>
                      ...
                      <div style={{ color: GREEN, fontWeight: 800, fontSize: 17, marginTop: 2 }}>from {fmt(svc.base)}</div>
                ...
                <div key={ex.id} style={{ background: WHITE, borderRadius: 12, border: `1px solid ${BORDER}`, overflow: "hidden" }}>
                  <div style={{ background: `linear-gradient(135deg,${LIGHT_GREEN},${WHITE})`, padding: "16px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div>
                      ...
                      <div style={{ color: GREEN, fontWeight: 900, fontSize: 18 }}>+{fmt(ex.price)}</div>
                    </div>
                  </div>
...
function LoginScreen({ clients, onLogin, onAdmin, onGuest }) {
  ...
  return (
    <div style={{ maxWidth: 420, margin: mobile ? "0 auto" : "56px auto", padding: "16px" }}>
      <div style={{ background: WHITE, borderRadius: 18, border: `1px solid ${BORDER}`, padding: mobile ? "24px 20px" : "36px", boxShadow: "0 8px 40px rgba(27,117,187,0.10)", marginTop: mobile ? 8 : 0 }}>
        ...
        <button style={{ ...S.btn(WHITE, BLUE, BLUE), width: "100%", marginBottom: 16, padding: "14px" }} onClick={onGuest}>Continue as Guest</button>
        <hr style={{ border: "none", borderTop: `1px solid ${BORDER}`, margin: "16px 0" }} />
        <button onClick={onAdmin} style={{ width: "100%", background: BG, color: MUTED, border: `1px solid ${BORDER}`, borderRadius: 9, padding: "12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          🔐 Admin Login
        </button>
...
function AdminLogin({ onLogin, onBack }) {
  ...
  return (
    <div style={{ maxWidth: 380, margin: mobile ? "0 auto" : "56px auto", padding: "16px" }}>
      <div style={{ background: WHITE, borderRadius: 18, border: `1px solid ${BORDER}`, padding: mobile ? "24px 20px" : "36px", textAlign: "center", boxShadow: "0 8px 40px rgba(27,117,187,0.10)", marginTop: mobile ? 8 : 0 }}>
            ...
            {!mobile && <>
              ...
              {isAdmin && <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ background: checkSuperAdmin(adminUser) ? `linear-gradient(135deg,${BLUE},#2196f3)` : LIGHT_BLUE, borderRadius: 8, padding: "6px 14px" }}>
                  <span style={{ fontSize: 12, color: checkSuperAdmin(adminUser) ? WHITE : BLUE, fontWeight: 800 }}>
                    {checkSuperAdmin(adminUser) ? "⭐ Super Admin" : "👤 Admin"}: {adminUser?.name}
                  </span>
                </div>
